...
class IGPMTitPubFetcher(scraps.Fetcher):
    ...
    def parse(self, content):
        ...
        taxa = tables[6].tr.td.contents[0].string.split(':')[1].replace('%', '')
        tipo = 'projecao'
        if taxa.strip() == '':
            taxa = tables[7].tr.td.contents[0].string.split(':')[1].replace('%', '')
            tipo = 'media'
        scrap = InflacaoTitPubScrap(data_atual=data_atual, taxa=taxa, tipo=tipo)
        return scrap

class IPCATitPubFetcher(scraps.Fetcher):
    ...
    def parse(self, content):
        ...
        tipo = 'projecao'
        if taxa.strip() == '':
            taxa = tables[8].tr.td.contents[0].string.split(':')[1].replace('%', '')
            tipo = 'media'
        scrap = InflacaoTitPubScrap(data_atual=data_atual, taxa=taxa, tipo=tipo)
        return scrap
